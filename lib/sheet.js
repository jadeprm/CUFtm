import { guessFromPosition } from './departments.js';
import { normaliseAccess } from './auth.js';
...
export function rowsToPeople(rows) {
  ...
  for (let i = headerAt + 1; i < rows.length; i++) {
    ...
    people.push({
      ...
      position: col.position === -1 ? '' : clean(row[col.position]),
 ...
 *
 * What the sheet owns: who exists, their access level, position, nickname.
 * What the app owns and sync must never trample: passwords, profile pictures,
 * language choice, an admin's department assignment, and the suspended flag.
 *
...
export async function syncPeople(sql, people) {
  ...
  for (const person of people) {
    ...
    if (!existing) {
      ...
      await sql`
        INSERT INTO users (username, nickname, sheet_name, display_name, position, access,
                           department, is_head, active)
        VALUES (${person.username}, ${person.nickname}, ${person.sheetName}, ${person.sheetName},
                ${person.position}, ${person.access}, ${guess.department}, ${guess.isHead}, true)
      `;
      added++;
      continue;
    }
    ...
    await sql`
      UPDATE users SET
        ...
        department   = COALESCE(${existing.department}, ${guess.department}),
        is_head      = ${existing.department ? existing.is_head : guess.isHead},
        active       = true,
        updated_at   = now()
  ...
  return { added, updated, deactivated: deactivated.map((r) => r.username) };
}
