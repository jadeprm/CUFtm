...
export async function currentUser(request, sql) {
  ...
  // Every visibility decision reads this, so it is loaded once here rather
  // than fetched again inside each rule.
  row.departments = await departmentsOf(sql, row.username);
  return row;
}
